...
{[(P,env_1)||(Q,env_2),env,k,ch]\trans{\alpha}[P',env_1',k[\hole\|Q],ch']}
}\\\\
\multicolumn{2}{c}{
\inference[Com(2)]
{[Q,env_2,k,ch]\trans{\alpha}[Q',env_{2}',k[P\|\hole],ch']}
{[((P,env_1)||(Q,env_2),env,k,ch]\trans{\alpha}[Q',env_{2}',k[P\|\hole],ch']}
}\\\\
\multicolumn{2}{c}{
\inference[Com(3)]
{[P,env_{1},k[\hole\|(Q,env_{2})],ch]\trans{\overline{\alpha}}[P',env_{1}',k',ch']\\
 [Q,env_{2},env,k[(P,env_{1})\|\hole],ch]\trans{\alpha}[Q',env_{2}',k'',ch'']}
{[(P,env_1)\|(Q,env_2),k,ch]\trans{(\overline{\alpha},\alpha)}[(k[(P',env_{1}')\|(Q',env_{2}')]),\hole,ch]}
}\\\\
\multicolumn{2}{c}{
\inference[Instance]
{}
{[A(\langle v\rangle),env[A\leftarrow P(\langle x\rangle)],k,ch]\trans{}[(k[(P,env[A\leftarrow P][\langle x\rangle\leftarrow\langle v\rangle])]),env[A\leftarrow P(\langle x\rangle)],\hole,ch]}
}\\\\
\multicolumn{2}{c}{
\inference[Constant]
{[A(\vec{v}),env[A\leftarrow P(\vec{x})],k,ch]\trans{}[(P,env[A\leftarrow P(\vec{x})][\vec{x}\leftarrow\vec{v}]),env[A\leftarrow P(\vec{x})],k,ch]\\
[P(\vec{x}),env[A\leftarrow P][\vec{x}\leftarrow\vec{v}],k,ch]\trans{\alpha}[P',env[A\leftarrow P(\vec{x})][\vec{x}\leftarrow\vec{v}],k,ch]}
