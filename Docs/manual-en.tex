...
Linveness properties require a subset containg no accepting states since an automaton specifing a liveness property moves to some state by all reading input.
To ensure that \(\alpha\) such that for any inifinte sequence \(\beta\) the automaton rejects \(\alpha\beta\), does not exist.
...
the existance of \(C_{i}\) and \(T_{ij}\) means the existance of an invariant $I$ which follows oblications O1\textendash O2 in \cite{Alpern:1989}.
