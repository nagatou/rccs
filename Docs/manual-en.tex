...
First, we must show that $m$ can make some transition from its start state upon reading the first state in a history.
  \begin{definition}[Transition Basis]
    \begin{displaymath}
Init_{\pi}\Rightarrow\vee_{j}(q_j\in Q\mbox{ and }T_{0j})
    \end{displaymath}
  \end{definition}
Second, we must show that $m$ can always make a transiton upon reading subsequent states in a history.
If $m$ is in state \(q_i\) then the model state read by $m$ satisfies a correspondence invariant \(C_i\).
To avoid an undefined transition, any atomic action \(\alpha\) that is then executed must transform the model state so that one of the transiton predicates \(T_{ij}\) emanating from \(q_i\) holds.
This is guaranteed by 
  \begin{definition}[Transition Induction]
...
Finaly, we present conditions to ensure that $m$ cannot reject \(\sigma\) by failing to enter accepting state infinitely often (scenario (2) above).
  \begin{definition}[Knot Exit]
For all \(\alpha (\in {\cal A})\), and for all $i$ \((q_i\in Q)\),
    \begin{displaymath}
\{C_i\}\alpha\{\vee_{j:q_j\in Q}T_{ij}\}
    \end{displaymath}
  \end{definition}
  \begin{definition}[Knot Variance]
...
\infer[\mbox{Consequence}]
      {\{p\}P\{q\}}
      {\{p\rightarrow p'\}P\{q'\rightarrow q\}}
  \end{displaymath}
  \begin{displaymath}
...
\item \(\{\mbox{pre}_1\wedge\mbox{pre}_2\}\bijection{a}:P\|a:Q\{\mbox{post}_1\wedge\mbox{post}_2\}\) holds
whenever \(\{\mbox{pre}_1\}\bijection{a}:P\{\mbox{post}_1\}\) and \(\{\mbox{pre}_2\}a:Q\{\mbox{post}_2\}\) are taken from the proof of \(\{p_i\}P\{q_i\}\) and \(\{p_j\}Q\{q_j\}\).
  \end{enumerate}
We shall need the following axiom to establish cooperation:
  \begin{displaymath}
\infer[\mbox{Communication}]
      {\{\Truerm\}[a(x)\colon P\|\bijection{a}(y)\colon Q]\{x=y\}}
      {}
  \end{displaymath}
%
%%% Proving Properties
...
When two processes cooperate through an input action \(a(x)\) and a matching output action \(\bijection{a}(v)\), a state of a model is chenged by the assignment \(x:=v\) which is atomic.
The assignment as \(x:=v\) denotes a state change from \(env\) to \(env[v/x]\).

The following explains the correspondence between each predicate in \secref{sec:CorrespondenceBtweenModelsAndFormulae} and the modelling language in \secref{sec:OPERATINAL_SEMANTICS}.
A correspondence invariant \(C_i\) intuitively predicates:
  \begin{itemize}
\item a model changes its state to \(s_i\) and
\item an automaton $m$ enters \(q_i\) reading \(s_i\).
  \end{itemize}
\(C_i\) corresponds to a valuation and the fomula of the valuation becomes \((v_1=a_1)\wedge\cdots\wedge (v_n=a_n)\) when a set of state variables is \(\{v_1,\cdots,v_n\}\).
We correspond a valuation to an environment in \NHK.
Therefore, we regards an environment \(env_i\) as a predicate:
  \begin{displaymath}
Env_i\equiv({\displaystyle\wedge_{i}}(v_i=a_i)).
  \end{displaymath}
\(Init_{\pi}\) is \(Env_0\) which is \(\Truerm\).

Transition predicate \(T_{ij}\) is deriverd from transition function.
\(T_{ij}\) holds for all \(s_j\) such that \(q_j\in\delta(q_i,s_j)\).
Each transition of an automaton is indicated with form \((\mbox{if}\ (B_i)\ (P_i)\ (Q_i))\).
Thus, we correspond \(T_{ij}\) to
  \begin{displaymath}
Env_j\models B_i.
  \end{displaymath}

To show that a model satisfies a property, we demonstrate proof obligations in \secref{sec:CorrespondenceBtweenModelsAndFormulae}.
%
