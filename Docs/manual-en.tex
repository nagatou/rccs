...
\vspace*{3cm}
\begin{center}
\includegraphics[scale=0.7]{logo-theta.jpg}
...
\\\\
\inference[IF(2)]
{(P,env)\trans{[\alpha,\overline{\alpha}]}(P',env') & env'\models B&\mbox{is-if}(E_F)}
...
secondDFS(m,f,S1,S2){
   if [m,f] is not a member of S2
    ...
    \end{lstlisting}
  \end{center}
{\tt m'} and {\tt f'} are produced following the semantics in \secref{sec:OPERATINAL_SEMANTICS} and \secref{sec:SemOfProp}.
